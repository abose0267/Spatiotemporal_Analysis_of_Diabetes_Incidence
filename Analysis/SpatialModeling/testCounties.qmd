...
data2$CountyFIPS <- sprintf("%05s", paste0(data2$CountyFIPS))


averaged_data2 <- data2 %>%
  group_by(CountyFIPS) %>%
...
merged_data <- merge(averaged_data, data2_relevant, by = "CountyFIPS", all.x = TRUE, all.y = TRUE)

merged_data <- subset(merged_data, !(StateAbbr %in% c("AK", "HI")))

# Calculate correlation matrix
cor_matrix <- cor(merged_data %>% select(-CountyFIPS, -StateAbbr), use = "complete.obs")  # Handling missing values by using complete observations

# Output variable names (column names) used for correlation matrix
cat("Variables used for correlation matrix:\n")
print(names(merged_data %>% select(-CountyFIPS, -StateAbbr)))
...
spatial_data$CountyFIPS <- sprintf("%05s", paste0(spatial_data$GEOID))

cleaned_spatial = data.frame()


selected_columns <- spatial_data[, c("CountyFIPS", "geometry","INTPTLAT","INTPTLON")]
cleaned_spatial <- data.frame(selected_columns) %>% na.omit()
```
```{r}
merged_data <- merge(merged_data, cleaned_spatial, by = "CountyFIPS", all.x = TRUE, all.y = TRUE)
```
...
ggplot() +
  ...
  scale_fill_gradient(name = "Diabetes Crude Prev", low = "lightblue", high = "darkblue",limits=c(0,25)) +
...
# Plot the spatial distribution of DIABETES_CrudePrev
ggplot() +
  geom_sf(data = merged_sf, aes(fill = LPA_CrudePrev), color="black",size=0.2) +
  scale_fill_gradient(name = "LPA Crude Prev", low = "lightblue", high = "darkblue",limits=c(0,20)) +
  labs(title = "Spatial Distribution of Diabetes Crude Prevalence")

# Plot the spatial distribution of GWR results
ggplot() +
  geom_sf(data = gwr_results_sf, aes(fill = LPA_CrudePrev), color = "black", size = 0.2) +
  scale_fill_gradient(name = "Predicted Diabetes Crude Prev", low = "lightblue", high = "darkblue",limits=c(0,20)) +
  labs(title = "GWR Predicted Diabetes Crude Prevalence using GWR")
```
...
mse = mean((gwr_results_sf$residual)*(gwr_results_sf$residual)) %>% sqrt()
print(mse)
```
...
merged_df <- as.data.frame(merged_spatialdf)
...
rmse <- sqrt(mean((rf_pred - test_df$DIABETES_CrudePrev)^2))
cat("RMSE:", rmse, "\n")
...
### Gradient Boosted Model:
The Gradient Boosted Model shows a tendency to smooth out the spatial variability seen in the Random Forest outputs. While this may imply a less nuanced capture of local occurrence, it could also indicate a more generalized understanding of the data, which might be useful for broader public health planning. The GBM's residuals also display a good fit overall but, but like the Random Forest model, shows room for refinement in capturing local variance.

### Comparative Analysis:
The Geographically Weighted Regression (GWR) model effectively captures local variations in diabetes prevalence, highlighting the impact of regional factors. In comparison, the Gradient Boosted Model (GBM) provides a more generalized overview, potentially smoothing over local nuances. GWR's strength lies in its spatially detailed predictions, while GBM's smoothed predictions may be beneficial for broader trend analysis. For public health strategies, GWR's detailed spatial insights could be more preferred. On the other hand, the GBM's general predictions across larger areas could be more appropriate for overarching health policies.